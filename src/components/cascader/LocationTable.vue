...
<script type="text/ecmascript-6">
  ...
  import {
    ...
    ON_REFRESH_DATA
  } from 'kalix-vue-lib-qiao/src/components/common/event.toml'

  export default {
    name: 'kalix-location-table',
    props: {
      isFixedColumn: {
        type: Boolean,
        default: false
          ...
          case 'edit': {
            ...
            setTimeout(() => {
              ...
              if (typeof (this.$refs.kalixDialog.init) === 'function') {
                ...
                console.log('==table=====', this.dialogOptions, row)
                if (this.dialogOptions && this.dialogOptions.row) {
                  this.dialogOptions.row = row
                }
                this.$refs.kalixDialog.init(row)
              }
            }, 20)
