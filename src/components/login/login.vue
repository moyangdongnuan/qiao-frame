    ...
    div.from-warpper
      div.form
        ...
        v-bind:show-message="false")
          ...
          el-form-item.kalix-form-item(prop="vcode")
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    data() {
      // window.identifyCode
      // 自定义验证
      let validateIdentifyCode = (rule, value, callback) => {
        callback()
        // 登录验证码验证
        // if (!value || value === '') {
        //   callback(new Error('请输入验证密码'))
        // } else if (value !== window.identifyCode) {
        //   callback(new Error('验证密码输入错误！'))
        // } else {
        //   callback()
        // }
      }
    ...
    methods: {
      ...
      makeCode(o, l) {
        ...
        window.identifyCode = this.identifyCode
