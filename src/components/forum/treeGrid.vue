...
<script>
  export default {
    ...
    activated() {
      ...
      this.$KalixEventBus.$on(this.$KalixEventConfig.ON_REFRESH_DATA, this.refresh)
    },
    deactivated() {
      console.log(this.bizKey + '  is deactivated')
      this.$KalixEventBus.$off(this.$KalixEventConfig.ON_SEARCH_BUTTON_CLICK)
      this.$KalixEventBus.$off(this.$KalixEventConfig.ON_REFRESH_DATA)
    },
    computed: {
      tableWidth() {
    ...
    mounted() {
      ...
      this.$KalixEventBus.$on(this.bizKey + '-' + 'KalixDialogClose', () => {
        this.whichBizDialog = ''
      })
      //  绑定表格 icon 图标
